...
pub fn initFromScalar(s: Scalar, mt: MatType) !Self {
    const ptr = c.Mat_NewFromScalar(s.toC(), @intFromEnum(mt));
    return try Self.initFromC(ptr);
}
...
pub fn size(self: Self) []const i32 {
    ...
    return v.val[0..@intCast(v.length)];
}
...
pub fn reshape(self: Self, cn: i32, rows_: usize) !Self {
    const ptr = c.Mat_Reshape(self.ptr, @intCast(cn), @intCast(rows_));
    return try initFromC(ptr);
}
...
pub inline fn toCStructs(mats: []const Self) !c.Mats {
    const len: i32 = @intCast(mats.len);
