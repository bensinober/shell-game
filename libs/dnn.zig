...
fn strCompare(str: []const u8, comp: []const u8) bool {
    if (str.len != comp.len) {
        return false;
    }
...
pub const Net = struct {
    ...
    pub const BackendType = enum(i32) {
        ...
        pub fn toEnum(backend: []const u8) BackendType {
            if (strCompare(backend, "halide")) {
                return .halide;
            } else if (strCompare(backend, "openvino")) {
                return .open_vino;
            } else if (strCompare(backend, "opencv")) {
                return .open_cv;
            } else if (strCompare(backend, "vulkan")) {
                return .vkcom;
            } else if (strCompare(backend, "cuda")) {
                return .cuda;
            } else {
                return .default;
            }
    ...
    pub const TargetType = enum(i32) {
        ...
        pub fn toEnum(target: []const u8) TargetType {
            if (strCompare(target, "fp32")) {
                return .fp32;
            } else if (strCompare(target, "fp16")) {
                return .fp16;
            } else if (strCompare(target, "vpu")) {
                return .vpu;
            } else if (strCompare(target, "vulkan")) {
                return .vkcom;
            } else if (strCompare(target, "fpga")) {
                return .fpga;
            } else if (strCompare(target, "cuda")) {
                return .cuda;
            } else if (strCompare(target, "cuda_fp16")) {
                return .cuda_fp16;
            } else {
                return .cpu;
            }
